...
case "$GENOME" in
  hg38)      G=hg38;  UCSC_DB=hg38; CHROM_RE='chr([1-9]|1[0-9]|2[0-2]|X|Y|M)'; BLACKLIST_URL="$HG38_BLACKLIST_URL" ;;
  t2t-chm13) G=chm13; UCSC_DB=hs1;  CHROM_RE='chr([1-9]|1[0-9]|2[0-2]|X|Y|M)'; BLACKLIST_URL="$CHM13_BLACKLIST_URL" ;;
...
# --------------------------------------------------------------------------- #
# Interval bins (autosomes chr1-22 + chrX, chrY, chrM)
# --------------------------------------------------------------------------- #
if ! have "$BINS"; then
...
# DELFI-specific chrom.sizes: only centromere-bearing chromosomes (chr1-22, X, Y).
# DELFI iterates every contig here and (when a gap file is used) requires a centromere
# per contig, so chrM and unplaced/alt contigs must be excluded.
if ! have "$SUPP_DIR/${G}.delfi.chrom.sizes"; then
  echo "  [gen ] ${G}.delfi.chrom.sizes (chr1-22, X, Y)"
  grep -wE 'chr([1-9]|1[0-9]|2[0-2]|X|Y)' "$SUPP_DIR/${G}.chrom.sizes" > "$SUPP_DIR/${G}.delfi.chrom.sizes"
fi
...
# --------------------------------------------------------------------------- #
# Assembly gaps (DELFI centromere/telomere masking)
#   hg38: finaletoolkit gap-bed -> BED4 with centromere/telomere annotations.
#   chm13: T2T-CHM13 is gap-free and has no finaletoolkit gap preset, so DELFI runs
#          WITHOUT a gap file (params.t2t-chm13.yaml sets neither delfi_gap_file nor
#          delfi_gap_reference_genome); centromeres are excluded by mappability+blacklist.
# --------------------------------------------------------------------------- #
if [[ "$G" == "hg38" ]] && ! have "$SUPP_DIR/hg38.gap.bed"; then
